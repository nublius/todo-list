...
export const Controller = (function() {
	...
	const initDisplay = () => {
		...
		taskEditForm = document.querySelector("#task__edit__form");
	};
	...
	const loadProjectTasks = (projectTitle) => {
		...
		for (let task of tasksArray) {
			...
			settingsTaskButton.classList = "task__edit__button";
			settingsTaskButton.id = task.id;
	...
	const editTask = () => {
		taskEditForm.addEventListener("submit", (event) => {
			event.preventDefault();
			taskEditHandler();
			TaskEditor.getModal().close();
			tasksContainer.innerHTML = "";
			loadProjectTasks(currentProject.title);
			TaskEditor.init(); // <-- Move this **AFTER** loadProjectTasks
			taskEditForm.reset();
		});
	};

	const removeTask = (targetTaskId) => {
		let removeTarget = document.getElementById(`${targetTaskId}`);
		removeTarget.remove();

		const removeItemTarget = currentProject.findToDo(targetTaskId);
	...
	const taskEditHandler = () => {
		const title = taskEditForm.title.value;
		const description = taskEditForm.description.value;
		const dueDate = taskEditForm.dueDate.value;
		const id = TaskEditor.returnId();
